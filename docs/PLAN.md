...
| D29 | **Custom is a pencil beside the unit picker, not the last entry in it — and leaving it no longer empties the box** | `custom` was never a unit. Every other entry in that `<select>` says what the *numbers* mean; `custom` said what the *fields are*, and a list of units cannot show that difference. It also set the row's geometry: a `<select>` takes its width from its widest option, so the rarest choice was making the picker **72px**; with only real units left it is **56px**, measured. It is a 24px icon toggle now, pressed while custom is on, and the picker keeps showing the unit the box would otherwise be in — so choosing a unit there is the second way out, for an author who knows which unit they want rather than only that they are done with CSS values. **The toggle exposed a defect in a decision rather than in code.** `switchUnit()` cleared the whole box on the way out of custom, deliberately: `calc(100% - 2rem)` has no number for a number field, and keeping the parseable sides while dropping the rest makes the outcome depend on what each side held. That reasoning was written when leaving custom meant opening the unit list and naming a unit — an act that reads as *re-express these*. One press of a toggle labelled `Use a number and a unit` does not read as *and discard everything*, and it takes two clicks to destroy four good values. So each side is now kept if it parses and cleared if it does not: which sides emptied is visible in the four fields, where four values vanishing at once is not. Measured end to end — a box holding `calc(100% - 2rem)` beside `25px`, `25px`, `8px` keeps the three and drops the calc; a box of four numbers survives the round trip untouched. |
