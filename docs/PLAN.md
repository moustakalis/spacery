...
- **B. Responsive controls extension** — adds per-breakpoint padding and margin to any
  block declaring `supports.spacing`, via a namespaced attribute and server-side CSS, on a
  mechanism that is not specific to spacing. Block gap was spiked and deferred (D5). This
  is the part core cannot easily obsolete, because it is N-breakpoint by design.
...
**Spacery never edits the default tier.** Core's ordinary spacing controls already *are*
the default tier, so a Spacery field for it would duplicate them for no gain. The panel
therefore always edits one of the breakpoints: above every band the canvas reports no
tier, and `useSelectedTier()` falls back to the widest one rather than to nothing, so the
panel is always editable. An earlier draft answered that case with a hint telling the
author to resize the canvas; D17 removed it, because the author wanted to set a value and
the hint sent them somewhere else first.
...
| Min PHP | **8.1** | Readonly properties are the newest feature used. Was 8.2 until D41. |
...
Settings screen on its own top-level admin menu, a React app over `register_setting( show_in_rest )`
...
- **`wp i18n make-pot` cannot read TypeScript.** Pointed at this codebase it extracted 24
  strings where there are 84, with no warning. `bin/make-pot.sh` transpiled first and
  refused to write a POT when that produced nothing. **Superseded by D38**, which scans the
  built bundles instead — WP-CLI reads those — and drops the transpile step entirely.
...
| D4 | **Minimum WordPress 7.1, PHP 8.2** *(PHP lowered to 8.1 by D41)* | `settings.viewport`, the always-iframed editor and apiVersion 3 with zero compat shims. The excluded install base shrinks every week. |
...
| D24 | **Spacery's data survives deletion unless the site opts out, and the opt-in is a checkbox on the settings screen** | §9 of `MANUAL-TESTING.md` asked the question and left it open: there is no `uninstall.php`, so `spacery_breakpoint_source` and `spacery_custom_breakpoints` outlive the plugin. Deleting them looked like ordinary tidiness and is not. The stored breakpoint set is the **key every stored block value resolves through** — `Generator::normalize()` walks the *current* set and silently skips slugs it does not know — so a site that loses its breakpoints and later reinstalls falls back to Spacery's preset, whose slugs are `desktop`/`laptop`/`tablet`/`mobile`. A custom set that used those names keeps resolving *at different widths*; one that used any other names has its values pruned. Both are silent, and both are the damage `rename-spike.md` refuses to risk for a rename — the reasoning does not change because the trigger is a deletion instead. Equally, a plugin that leaves rows behind with no way to say otherwise is its own small discourtesy, and "we kept your data because we decided you wanted it" is not a defence. So: a third registered option, `spacery_delete_data`, **off by default**, surfaced as one checkbox under *When you delete Spacery*, whose help text states the cost of each answer rather than describing the control. `uninstall.php` reads that option and nothing else — no autoloader, no plugin classes, literal option names, because it runs against files that are about to disappear; `OptionsTest` guards that seam the way `BreakpointPatternsTest` guards D19's. Two things were found while building it and are worth keeping: the checkbox rides the screen's existing save cycle rather than writing on click, because a screen with two save models is one where nobody knows which half is committed — and `saveHint()` had to learn about it, having briefly lit a primary Save button beside the words "No changes to save." The option stores `'1'`/`'0'` rather than a boolean because WordPress writes boolean false into a varchar column as `''`, which `rest_is_boolean()` rejects, so `/wp/v2/settings` answered `null` for exactly the site that had opted out — measured, then fixed. Post content is never touched, by design (§3.1). || D25 | **The screen's words are written to `design-system.md` §7, and that outranks the wording drawn in `settings-screen.png`** | The drawings are the source of truth for this screen's *layout*, and §2 of `CONTRIBUTING.md` says to read them before touching it. They are not the source of truth for its *sentences*: their captions were written to explain a proposal to a reader, not to be read in a 250px column by someone mid-edit. Audited against the live screen, 125 strings in, the faults were consistent enough to be rules rather than fixes — a second sentence explaining why Spacery works this way, an internal named where the author cannot act on it, the passive voice, the code's account of a failure instead of the site's. `### Words` records the six, and the one exception that earns a recital. So where a drawing's caption and a §7 rule disagree, the rule wins and the drawing is not "corrected" back; 27 strings were rewritten on that basis, three of them drawn verbatim. **`### Measure` is the half that was measured rather than argued.** Two `CardBody` sentences ran to **128 characters on one line** at a 1502px viewport while the breakpoint table's guidance line beside them read comfortably — the difference being a `maxWidth: '420px'` nobody had generalized. Capping the rest at 420px brought every line on the screen inside the 45–75 band. The same measurement found the opposite failure at the other end of the screen and it is the sharper constraint: the `UP TO` column is a fixed **130px**, so help under a width field wraps at **~20 characters a line**, and the conflict message — drawn in `settings-screen.png` §C as a five-line ribbon under a one-line field — rendered as four. Anything that can land there is now written to fit two lines, which the `Covers` cell one column over makes affordable: it already states the consequence ("Nothing — no screens left"), so the message need not. Greek was carried across in the same pass and is the reason the cap is two lines rather than one. |
| D26 | **How a line breaks is a stylesheet's job, not an editor's** | D25 capped how wide a line may be and left the second half of the same fault. A sentence a few characters past one line wraps to a widow: `Your breakpoints are kept, so reinstalling leaves your spacing exactly as it is.` broke **70 / 9** on the live screen, and a row's message in the 130px column broke **13 / 8**. Both are inside the measure. The fix is `text-wrap: balance` rather than shorter strings, and the reason is translation: a break point is a *rendered width*, so a sentence fitted by hand to fall well in English falls somewhere else in Greek — the copy fix cannot be right in two languages at once, and the browser can. Two mechanics, both found by the rule not working: it is **not inherited** (a declaration on `#spacery-settings` left every descendant computing `pretty`, with nothing on the page setting `pretty`), and it needs a **block** container, which an inline `Text` does not have — so on the admin screen the three `maxWidth: 420px` wrappers carry it, putting the width rule and the division rule on the same element. The audit that found it also found two things the string-level pass had missed, both because it was reading one text node at a time: the ruler's note ran to **101 characters on one line**, because `emphasise()` splits its sentence into several nodes and no wrapper capped it; and in the inspector, three sibling `Text` elements — inline — flowed as **one paragraph with no space between the sentences**, rendering `narrower screens.In Spacery they also`. The notice is now a column of blocks. |
| D27 | **The `Covers` column states its lower edge as exclusive, in words, and there is only one function that says what a tier covers** | Reported from the screen: with rows at 1300px and 888px, `Covers` read `888px – 1300px` on one and `450px – 888px` on the other, naming 888px twice and reading as an overlap. Overlap is impossible — `BreakpointSet::media_queries()` emits `@media (888px < width <= 1300px)`, the disjoint shape D13 chose to match core — so 888px is matched by the *narrower* tier and by that one alone, and the column was describing a thing that cannot happen. A dashed range has no way to say which end is open, so the column now says `over 888px, up to 1300px`: the same words the ruler's description already used. `range()` is deleted rather than corrected. It existed only to be tidier than `band()` in a narrow column, the two were two readings of one fact, and they drifted the moment the second one existed — `coverage()`'s own docblock claimed it "goes through the same `band`" while calling `range()`, which is the **seventh** false claim this repo has made about itself and the second found in a docblock describing the line beneath it. The prose form is 26 characters and fits the 284px column on one line, measured. **The obvious alternative was proposed and measured down:** writing the narrower edge as `previous + 1` (`889px - 1300px`) is the convention everyone reaches for, and it reintroduces the gap these range queries exist to remove. Probed by sizing an iframe to each width and reading `matchMedia`: at **888.25px and 888.5px** the real query `(888px < width <= 1300px)` **matches**, while `(max-width: 888px)` and `(min-width: 889px)` both miss -- so the label would tell an author that nothing covers a width something is already styling. Widths are not integers (browser zoom alone produces them: a 1512px screen at 110% is 1374.5px), and for an `em` breakpoint `+1` is not even defined -- one pixel, or one em? Bootstrap's `767.98px` is the same wound, bandaged. The exclusivity has to be stated, not arithmetic'd around; the compact `>888px - 1300px` and the exact `888px < w <= 1300px` were both offered and the prose was chosen, because it says it in English and matches the ruler's own sentence. |
| D28 | **The inspector gets native spin arrows and one 10px exception to the type floor; everything else stays on WordPress's scale** | Reported from the panel: the type looked large beside Elementor's, and the number fields had no arrows. Measured before changing anything, and the first half of the report did not survive it — Spacery's `PADDING` / `MARGIN` and side labels are **11px / 600 / uppercase, byte-identical to core's own Padding and Margin labels in the Dimensions panel in the same sidebar**, and its fields are already tighter than core's (13px in a **32px** field against core's **40px**). Spacery is not oversized against WordPress; Elementor is simply a denser design language, and §2's floor exists to stop this plugin growing a type scale of its own. So the scale holds, with exactly one exception: **the four side labels drop to 10px caps**, because four fields across a 248px panel are **59px each** where core spends the same width on one Vertical/Horizontal pair, and at 11px `BOTTOM` renders **49px in a 49px box** — flush, with nothing spare. At 10px it is 45px. Recorded and not fixed: Greek `ΑΡΙΣΤΕΡΑ` is 55px at 11px and 50px at 10px, clipped either way, which is a layout problem and not a type one. **The arrows are `spinControls="native"`, and the alternative was measured down:** core's `custom` spin buttons render as a suffix two 24px buttons wide — **60px inside a 59px field, leaving the number 12px** — so they are usable only in a two-sided box. Native arrows live inside the input and cost nothing until it is hovered. They also made a floor necessary: `Generator::is_value()` allows a leading `-`, so a negative padding is emitted and silently dropped by the browser, and an arrow makes that reachable by holding a key rather than by typing a minus sign. `min={0}` on padding, nothing on margin, which may legitimately be negative. The extension gains its first stylesheet for the one rule a prop cannot reach, with `!important` against the same thrice-emitted emotion class `CONTRIBUTING.md` already documents for borders. |
| D29 | **Custom is the `css` entry in the unit picker, not the word `custom` and not a toggle beside it — and leaving it no longer empties the box** | Two revisions in one entry, because the first was wrong and the record should say so rather than read as though it were not tried. **The word was the problem, and then the pencil was.** `custom` is six characters in a list of three-character units, and a `<select>` takes its width from its widest option, so the rarest choice was making the picker **72px**. Moving it out to a 24px pencil toggle fixed the width and broke something better: the mode and the units then lived in two controls that could not show one state between them — the picker still displayed a unit while the fields took whole CSS values, so the row asserted `px` about fields that were not in px. The entry belongs *in* the list, where choosing it is the same act as choosing a unit, and only its label needed to shrink: **`css`**, three characters like `rem`, naming what the fields will take rather than describing the mode. Picker measured at **56px**, the width it takes to show a unit and no more. An icon inside the list was asked for and is not available: a native `<select>` renders text, and reaching an icon means replacing `SelectControl` with `CustomSelectControl` — a second experimental component in the ambient types, different keyboard and ARIA behaviour, for a glyph that would say less than the word. **Leaving custom keeps what parses**, which survives both revisions on its own reasoning: `switchUnit()` used to clear the whole box, on the argument that keeping the parseable sides and dropping the rest makes the outcome depend on what each side held. True, and the lesser problem — four re-labelable values destroyed to avoid an inconsistent-looking outcome is the larger one, and *which* sides emptied is visible in the four fields where four values vanishing at once is not. It also makes leaving custom behave like every other unit change, which re-labels rather than clears. Measured: `calc(100% - 2rem)` beside `25px`, `25px`, `8px` keeps the three and drops the calc; four numbers survive `px → css → px` untouched and become `25rem` on `css → rem`. |
| D30 | **The plugin's one piece of motion: in `css` mode the focused field takes the room** | Four fields share 236px, so each is 59px — about nine characters, which is fine for `24px` and useless for `calc(100% - 2rem)`, the value this mode exists to hold. Widening all four for the one being typed in is the worse trade, so the focused field grows and the siblings give the space back on blur. `flex-grow: 2.7` is Elementor's own proportion for the same gesture, read off its panel: the focused field takes ~47% of the row and the other three share the rest, `g / (g + 3) = 0.47`. Here that is **112px against three of 41px**, measured. Accepted cost: `BOTTOM` is 45px at 10px and clips to its ellipsis at 41px while a sibling is focused, reverting on blur. Under `prefers-reduced-motion: reduce` the field still grows and only the tween goes — the width is the point. **Verifying it produced the session's sharpest trap.** In the real panel the width never moved, with the rule provably matching and `!important` set, which reads exactly like a cascade problem and is not one: **the browser window being measured was minimized, and a backgrounded tab never advances a CSS transition** — the property sits at its start value indefinitely. Suppressing the transition showed the correct end state immediately; the tween was then measured in headless Chromium on a fixture reproducing the row (59 → 71 → 81 → … → 112 across ~150ms, same 112). A property that will not animate in a background tab is not a broken property, and `flex-grow` interpolates fine. |
| D31 | **The breakpoint table's footer is part of the table: 16px below the closing rule, and inset with the columns** | Reported from the screen — *Add breakpoint* sitting on the last row's border. Measured: **0px** between the last row's bottom edge and the button, so it stood on the closing rule with nothing between them. The same measurement found a second fault nobody had reported: the button started at the card's content edge (**206px**) while every column above starts at **214px**, because `.spacery-table__row` carries `padding: 12px 8px` and the footer carried none — the button hung 8px left of the column it belongs under, and the guidance beside it ran 8px past the remove column's right edge. One cause, two symptoms. The inset is **matched rather than removed from the rows**: a conflicted row is tinted across the card's full width and its fields have to sit inside that tint, not flush against its edge. So the table keeps its 8px and the footer joins it — button now at 214, guidance ending at 1450 with the remove column, 16px of air above and 16px below to the card's edge, symmetric with the header's 16px on the other side. Holds in the conflict state and at a 700px wrap. **The audit that found it also found this document lying about itself, which is the ninth time:** §3's spacing table listed `Card padding 16px`, `Card header 12px / 16px` and `Table row 9px / 16px`; the screen renders **16/24**, **16/24** and **12/8**. Those were written from the proposal and never checked against the build. §3 now records measurements, says so, and adds the two rows it never had — the table head's 8px and the footer's. What the audit confirmed rather than corrected: every card is symmetric 16px top and bottom, every card's content starts on one left edge, cards are 20px apart, and no gap anywhere on the page is off the 4px grid. |
| D32 | **The ruler's ticks sit on their boundaries, and its bands are separated by a border rather than a gap** | Reported from the screen: *the coloured bar is clearly bigger than the value it is named by*. Two independent errors, both measured, and together they put a band edge 14–17px right of the tick that names it. **The tick was the larger one.** Each tick was a column — mark, then label — positioned at `left: at%` and pulled back by `translateX(-50%)`. That centres the **group**, and the group is as wide as its label, so the 1px mark landed **half a label-width early**: 16.8px at `480px`, 19.2px at `1280px`, always exactly half that tick's own text. The fix is a zero-width tick box: a box that has no width cannot be displaced by the length of its own text, so the mark falls on the boundary and the label overflows symmetrically under it. **The band was the smaller one, and its comment was a false claim of the kind this repo keeps finding.** The strip separated bands with `gap: 2px` and the code said, in as many words, that a border "would add width to every band and lie about the proportions" — the gap did exactly that. Four gaps made the row 100% + 8px, flex shrank every band to fit, and each boundary fell 1–3px short: measured `Mobile` ending at 260.1 where its own width says 262.8. A 2px white border drawn *inside* the band (`border-box`, already set) separates them without taking width from anyone. After: every tick within **0.5px** of its boundary — half the mark's own 1px — every band edge at **0.0px** drift, and the bands summing to exactly the strip's width. |
...
| D37 | **Custom mode is earned by what a box holds, never by what it inherits — and an inherited preset is shown by name** | Reported from the screen: set padding through core's own Dimensions control, open the Spacery panel, and the box is in `css` with `var:preset|spacing|50` in every field. Not an edge case — **core's padding control stores a preset by default**, so this was every tier of every block whose padding was set the ordinary way. D22 had extended its custom-first rule to inherited values, and that extension is the error: the rule is about what a box *holds*, because those values are the author's, they are on the page, and a box that cannot show one of them is in the wrong mode. An inherited value is none of those things — it belongs to a wider tier or to core's Dimensions panel, and it is already legible where it was set. **The harm was not cosmetic.** In `css` a field takes a whole CSS value, so typing `24` — the obvious thing, into a box the author did not put in that mode — stored `24`, which `is_value()` accepts (its `NUMBER` makes the unit optional, for `0`) and the browser drops. In `px` the same keystroke stores `24px`. So `resolveUnit()` asks `inheritedUnit()` about placeholders, which is `unitFor()` minus that one rule and can never return custom; the author can still pick it. **And the placeholder now reads the preset's resolved size, or its name when there is no size to read.** The first attempt showed the name always, and Nick rejected it with the obvious question — core's own panel says `20px`, so why can Spacery not? It can: core exports `getCustomValueFromPreset()`, which resolves a reference against the site's scale, and on Twenty Twenty-Five `var:preset|spacing|30` is `20px`. So a resolved preset is shown exactly like any other inherited length — `20` in a `px` box — and the **name** is the fallback for the ones that resolve to a `clamp()`, which four of that theme's seven sizes do and which has no number to put in a number field. Preferring the name everywhere would have been a worse placeholder chosen for the convenience of the minority case. Both measured on the live editor: `X-Small` → `20`, `Regular` → `Regular`. Only in a number field: in `css` the fields take the raw reference, because that is what the block stores and the only one of the three a person could type back in. **Two API shapes measured rather than assumed, because both fail silently:** `useSettings` returns one value per requested path, so a single path arrives wrapped in an array — a `.find()` over that matches nothing, falls back to the raw reference, and looks exactly like a fix that changed nothing. And `getCustomValueFromPreset()` returns `undefined` for a slug the site does not define, passes a non-preset straight back, and **throws** when `sizes` is undefined, so an array is always passed and the guard is `isValueSpacingPreset()` rather than a truthy check on the result. Verified live in both directions: inheriting a preset gives `px` and `Regular`, and typing `24` stores `24px`; holding `var:preset|spacing|60` at the tier still opens `css` with its help line. **Named but not fixed:** the allowlist accepts a unitless non-zero number, which can never render as spacing. `0` legitimately has none, so the rule is not simply wrong — tightening it means moving the TS table and `GeneratorTest`'s 38 PHP cases together. |
| D38 | **The POT describes the distributable, because its references are what name a language pack** | `_load_script_textdomain_from_src()` finds a script's translations at `WP_LANG_DIR/plugins/<domain>-<locale>-<md5>.json`, where the md5 is of the registered script's path relative to the plugin root — `build/settings.js`. `wp i18n make-json` names what it writes after the references in the `.po`, which come from this repository's POT. `bin/make-pot.sh` transpiled `src/**` and scanned that, so every reference named a source file and every payload this repository compiled was named after a path core never hashes — a file nothing would ever open, indistinguishable from a working one. Bundled handle-named payloads and a path passed to `wp_set_script_translations()` hid it, because that lookup happens first and only inside that path: Greek worked and nothing else would have. So the scan runs over a copy of what ships. Measured before committing to it: WP-CLI reads the minified bundles and extracts the identical 136 entries, losing one translator comment (`css`) where minification moved it off the front of the call, and the price is line references of `build/extension.js:1`, which tell a translator nothing. There is deliberately no stale-build guard — webpack's `compareBeforeEmit` leaves an unchanged bundle unwritten, so it keeps an old mtime and any "source newer than bundle" rule refuses on a fresh build; `pnpm run i18n:pot` builds first instead.  **Half of this row's premise was wrong, and it is corrected here on 21 September 2026 rather than rewritten, because the decision it produced was right.** The premise was that translate.wordpress.org names a pack's files after the references in this repository's POT, and that a pack would therefore never have loaded in any locale. It does not read this POT at all: `/languages` is in `.distignore` and out of `package.json#files`, so it reaches neither the zip nor SVN, and GlotPress extracts its own originals from `trunk/`. Measured: `svn ls` gives `trunk/` six entries and no `languages/`, while translate.wordpress.org holds 136 originals referencing `build/*.js`, `build/blocks/spacer/block.json`, `includes/**.php` and `spacery.php` — the same ten files with the same per-file counts as this POT. **`src/` has never been in the distributable**, so a pack would always have carried `build/` references whatever the POT said. What was genuinely broken was the repository's own compiled Greek and the CI test that installs it as a pack, which is what D38 fixed. The scan still has to describe the distributable, for a reason one step removed from the original: it is the local model of WordPress.org's extraction, and it is the only place a packaging mistake is visible before a release. That makes the **packaging list** the thing D38 depends on, which is why `bin/make-pot.sh` now checks its scan list against `package.json#files`. Fifteenth untrue thing, and the second of the never-true kind that reasons correctly to a right answer (§3quindecies was the first). |
| D39 | **Greek follows WordPress core wherever core has settled a term, and keeps English only where the corpus is empty** | Asked as "shouldn't the technical terms stay in English?", and the measurement answered a different question than the one asked. Three Greek articles on responsive design (including the Greek Wikipedia entry, whose **title** is the Latin-script `Responsive design`) show Greek tech prose borrowing freely and glossing rather than coining. But translate.wordpress.org's consistency tool for `el/default`, read on 21 September 2026, shows the WordPress corpus doing the opposite wherever it has a settled term: `Width` → `Πλάτος` (458 of the sampled rows), `Dimensions` → `Διαστάσεις` (64/64), `Margin` → `Περιθώριο` (55/57, and a glossary entry), and **in core alone, unanimously**, `Padding` → `Γέμισμα` (17/17), `Margin` → `Περιθώριο` (14/14), `Spacing` → `Διαστήματα` (11/11), `Spacer` → `Κενός Διαχωριστής` (7/7). Spacery was off-consensus on the two that matter most: it said `Εσωτερικό περιθώριο` for padding and `Εξωτερικό περιθώριο` for margin. **The harm is adjacency, not taste.** D34 puts Spacery's panel in the Styles tab immediately after core's Dimensions, so the same author read core's `Γέμισμα` and Spacery's `Εσωτερικό περιθώριο` for one CSS property inches apart — and Spacery's word for *padding* contained core's word for *margin*, an ambiguity that is real in the wild, where 16 projects translate `Padding` as bare `Περιθώριο`. Aligned. The other half of the answer is that **`Breakpoint` and `Viewport` return no results at all** — not in core, not in 500 rows of plugins, not in the glossary, not in any of the three articles. `σημεία διακοπής` is therefore a coinage rather than a divergence, and it stays. `Responsive` has a glossary entry, `προσαρμοζόμενη στις κινητές συσκευές`, which is five words, unusable in a 59px field, and semantically wrong here — Spacery is about widths, not devices — so the invented `αποκρίσιμο`, which appears nowhere in the corpus, was replaced by Latin-script `responsive` where the word is needed and by *ανά σημείο διακοπής* where a phrase reads better. `Γέμισμα` is also 7 characters against 19, which is the §3undecies column budget arguing the same way. **The rule this sets:** check the consistency tool before inventing a Greek term, and prefer core's answer over a better one, because the screen is read next to core's. |
| D40 | **Malformed block markup is refused here, because Spacery reaches it before core's own guard does** | The `spacery` attribute lives in the block comment delimiter, so it is whatever the last person to edit that post wrote — not what the inspector wrote. Nest a value one level too deep, `padding.top.x`, and `wp_style_engine_get_styles()` hands the side's value back as an **array** where a length belongs; it does not refuse it. `Generator::force()` then marked every declaration through a closure typed `string`, and under `declare( strict_types=1 )` that is an uncaught `TypeError` raised inside a `render_block` filter: a 500 on every page holding the block, caused by anyone who can edit it and fixable only by someone who can edit it back. **Core refuses the identical input one step further on** — `WP_Style_Engine_CSS_Declarations::add_declaration()` carries the comment *"Bail early if value is not a string. Prevents fatal errors from malformed block markup."* — so the input was survivable and this plugin is what made it fatal. That is the rule the row exists for: **wherever Spacery handles something before core does, it inherits core's obligation to be total about it**, and a type declaration is not a guard, it is a way of turning bad input into a fatal. Two smaller bypasses of the same allowlist went with it. `prune()` tested `is_string( $value ) && ! is_value( $value )`, so a number or a boolean fell through both arms and `flatten()` cast it back on the other side — `5` shipped as `padding-top:5`, past the one check on generated CSS and invisible to the editor preview, whose own walk keeps string leaves only. And a value was **stored as written while `is_value()` judged it trimmed**, so whitespace and (since `trim()` strips it) a NUL rode into the stylesheet on a verdict that had not seen them. A leaf is now a string that passed the allowlist, stored as the allowlist read it — which also makes the content hash address the trimmed value, so `10px` and `" 10px "` share one rule instead of two. **What makes this class hard to see:** the attribute is `mixed`, so it is not a type error; it is not a lint finding; and the E2E suite drives the inspector, which cannot produce the shape. Ten regression tests in `GeneratorTest` assert each malformed shape is refused *without throwing*, because the thrower is `render_block` and the symptom is the published page. `docs/security-audit.md` is the audit; F1 is this. |
| D41 | **Minimum PHP is 8.1, not 8.2** | D4's reason for 8.2 was *"typed properties, enums, readonly"* — PHP 7.4 and 8.1 features — so the floor was a default, never a requirement, and it turned away every site on 8.1 (about 18% of one March 2026 sample of 6,000+ WooCommerce stores). Measured, not grepped alone: PHPCompatibility's development branch at `testVersion 8.1` reports nothing across `spacery.php`, `uninstall.php` and `includes/`, and the same run at `8.0` flags the twelve readonly constructor properties, which proves the sniff reads this code. **The composer-installed PHPCompatibility (`phpcompatibility-wp` ^2.1, i.e. 9.x) cannot make this check**: it predates 8.1, so its `testVersion` is bookkeeping here, not a gate. CI gains a job that lints the shipped PHP on 8.1; the rest of the PHP job stays on 8.2 because PHPUnit 11 requires it. **8.0 was declined**: nine `readonly` declarations rewritten for a few percent of sites. WordPress 7.1 stays — it is the API (D4). Revisit if an 8.2 feature is ever wanted: raising a floor strands sites already running the plugin. |
