...
| D38 | **The POT describes the distributable, because its references are what name a language pack** | `_load_script_textdomain_from_src()` finds a script's translations at `WP_LANG_DIR/plugins/<domain>-<locale>-<md5>.json`, where the md5 is of the registered script's path relative to the plugin root — `build/settings.js`. translate.wordpress.org names a pack's files after the references in this repository's POT. `bin/make-pot.sh` transpiled `src/**` and scanned that, so every reference named a source file and no pack would ever have been opened, in any locale, with nothing to report it. Bundled handle-named payloads and a path passed to `wp_set_script_translations()` hid it, because that lookup happens first and only inside that path: Greek worked and nothing else would have. So the scan runs over a copy of what ships. Measured before committing to it: WP-CLI reads the minified bundles and extracts the identical 136 entries, losing one translator comment (`css`) where minification moved it off the front of the call, and the price is line references of `build/extension.js:1`, which tell a translator nothing. There is deliberately no stale-build guard — webpack's `compareBeforeEmit` leaves an unchanged bundle unwritten, so it keeps an old mtime and any "source newer than bundle" rule refuses on a fresh build; `pnpm run i18n:pot` builds first instead. |
